...
**Re-run cohorte v2@0.55 (fait)** : ancien run v1 purgé (`scripts/teardown_census_run.py` : MinIO+cache+image_assets CASCADE review_queue/dino, source_images → zero_crops ; garde-fou training_eligible=1). Nouveau run : 1899 candidats → **300 raws / 499 crops** (vs 733 v1), packaging/blanks largement coupés. Planches review `scripts/review_sheets_census.py`.

### Intégration pipeline (2026-06-05, PO go)

Décisions PO : (1) câblage par **paramètre explicite** (pas env) ; (2) **laisser les crops prod existants**, census seulement pour les nouveaux.

`detect_circles_multi`, `normalize_listing`, `normalize_listing_path` prennent un param `census: bool | None` — `None` (défaut) = résolu via `EURIO_CENSUS_DETECT` (rétro-compat bench/CLI), un bool explicite a priorité. `sources/_base/steps/detect_crop.run_detect_crop` passe **`census=(source_id == "ebay")`** : le pipeline eBay crope donc en census+gate **par défaut, en dur** (scopé eBay car la probe est 2€-only ; les autres sources `listing` restent strictes ; le scan device `normalize_device` n'est pas concerné). L'idempotence existante (`COUNT(present image_assets) > 0 → skip`) garantit que les raws **déjà cropés ne sont pas retouchés** ; les nouveaux + les ex-zero_crops passent en census+gate. `recrop_cohort_census` devient redondant pour les runs futurs (le pipeline le fait), gardé pour les rattrapages ponctuels. Tests : `test_census` 12 ✅, `test_lot_detection` 14 ✅ (les 4 échecs `test_normalize_listing` sur images synthétiques sont **pré-existants**, indépendants de ce changement).
