...
7. **Pas de changement ML training en v1** — le pipeline actuel tourne sur 5 coins, label utilisé = `eurio_id` directement. Le switch vers `COALESCE(design_group_id, eurio_id)` intervient quand on scale à 300-500 (Phase 2 du plan ML scalability).
