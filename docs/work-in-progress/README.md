...
| [`juge-et-banc/`](./juge-et-banc/README.md) | **Le corpus device servait de split de validation ET de juge** — les prototypes contre lesquels une photo était notée étaient la moyenne de ses propres photos. ✅ **Coupé le 2026-08-25 (7 lots), et chiffré** : la fuite valait **+14,7 pts** sur ce qu'elle avait vu (`p = 6,1 × 10⁻⁵`) et **−4,4** sur le reste ; le global rendait **+0,24 pt, `p = 1,0`** — soit « pas de fuite ». Corpus device **répliqué** (3 objets, 130,2 Mo). ⛔ Reste au PO : déployer `/ingest/quality-scores` (L6 non déployé), valider les 4 lignes d'`EXTRA_MAPPING`, générer les textures d'overlay, et **committer** (rien ne l'est). [`MATRICE.md`](./juge-et-banc/MATRICE.md) n'est **pas** commencée |
