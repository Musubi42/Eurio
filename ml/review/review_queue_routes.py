...
def list_queue(
    ...
    dino_min_denom: float | None = Query(default=None),
) -> list[ReviewItem]:
    ...
    # `source_alias='s'` : ici source_images porte l'alias `s`. Miroir exact
...
def dino_candidates_summary(
    ...
    try:
        return _lean.dino_candidates_summary(
            ...
            dino_era_only=dino_era_only,
            dino_min_denom=dino_min_denom,
            need_only=need_only,
        )
...
# ⚠️ L'auto-acceptation a DÉMÉNAGÉ le 2026-08-27 vers
# `serving/review_queue/auto_accept.py`, un module LEAN. Ce fichier-ci n'est
# pas chargeable par l'image du VPS (`cv2`, puis `training`) : la route
# était donc absente de l'OpenAPI de production — vérifié — alors que le VPS
# est le SEUL writer. L'auto-accept n'était exécutable nulle part, et c'est
# la vraie raison de son zéro depuis le 2026-07-08.
# Le déménagement a aussi retiré le filtre `rq.lane='auto_accept'` : la lane
# est figée à l'enqueue, le verdict se recalcule, et les deux avaient divergé
# de 960 contre 2 308. Ne la réintroduis pas ici.
