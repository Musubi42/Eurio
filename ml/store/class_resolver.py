...
Relocalisé depuis ``training/eval/class_resolver.py`` (C3, Direction A) : ce
module ne dépend que du stdlib (json/os/sqlite3/dataclasses/pathlib, +
``shared.env``/``store.resolve_db_path`` en lazy import) et ne lit QUE
``eurio.db`` — il vit donc dans ``store/`` pour être importable depuis
``store/funnel.py`` (lecture funnel servie sur l'image lean du VPS) SANS tirer
numpy/torch. ``training/eval/class_resolver.py`` ré-exporte ce module tel
quel pour préserver tous les usages existants (prepare_dataset,
compute_embeddings, seed_supabase, iteration_runner, lab_routes, etc.).
"""
...
def _default_db_path() -> Path:
    """DB locale canonique, honorant ``EURIO_DB_PATH`` (Model B : réplique).

    Ce module est ``ml/store/class_resolver.py`` (relocalisé C3) → ``parents[1]``
    == ``ml/``. Défaut legacy = ml/state/eurio.db quand l'env est absent.
    """
    from store import resolve_db_path
    return resolve_db_path(Path(__file__).resolve().parents[1] / "state" / "eurio.db")
