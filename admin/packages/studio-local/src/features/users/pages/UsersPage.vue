...
/**
 * Page Users — liste des utilisateurs du miroir local (peuplé par les logins OIDC) +
 * édition des rôles applicatifs. Rapatrié d'admin-vps (R1). Léger (eurio-api only).
 *
 * Scopes :
 * - Lecture : `users:read` (owner + admin)
 * - Édition : `users:manage` (owner uniquement) — le bouton "Modifier" est caché
 *   aux non-owner.
 *
 * Anti-lockout : l'API refuse de retirer le dernier owner. On affiche un message clair
 * côté UI si on tombe sur cette erreur.
 */
...
const canManage = computed(() => session.hasScope('users:manage'))
...
function isMe(u: UserRow): boolean {
  return u.id === session.principal?.user_id
}
